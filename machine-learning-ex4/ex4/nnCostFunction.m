...
y_1 = zeros(num_labels, m); % 10x5000

% y(i) is 1...10 according to the digit(10 to 0)
for i = 1:m
    y_1(y(i), i) = 1;
endfor

J = 1 / m * sum( sum( -y_1 .* log(a3) - (1 - y_1) .* log(1 - a3) ) );


Reg = lambda * (sum( sum ( (Theta1(:, 2:size(Theta1,2))).^ 2 )) + ...
...
for t = 1:m
    ...
    z2 = [1 z2']; % add bias 1x26
    del2 = (Theta2' * del3) .* sigmoidGradient(z2'); % 26x10 * 10x1
    
    del2 = del2(2:end); % remove 0 index 25x1
    
    Theta2_grad = Theta2_grad + del3 * a2; % 10x1 * 1x26
    Theta1_grad = Theta1_grad + del2 * a1; % 25x1 * 1x401
